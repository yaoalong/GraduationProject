...
public class Login {
    ...
    public Login(final String loginContextName, CallbackHandler callbackHandler)
        ...
        isKrbTicket = !subject.getPrivateCredentials(KerberosTicket.class)
                .isEmpty();
        ...
        // you can decrease the interval of expiration of tickets (for example,
        // to 3 minutes) by running :
        // "modprinc -maxlife 3mins <principal>" in kadmin.
        t = new Thread(new Runnable() {
            public void run() {
                LOG.info("TGT refresh thread started.");
                while (true) { // renewal thread's main loop. if it exits from
                               // here, thread will exit.
                    ...
                    if (tgt == null) {
                        ...
                        // determine how long to sleep from looking at ticket's
                        // expiry.
                        ...
                        nextRefreshDate = new Date(nextRefresh);
                        if (nextRefresh > expiry) {
                            LOG.error("next refresh: "
                                    + nextRefreshDate
                                    + " is later than expiry "
                                    + expiryDate
                    ...
                    if (now < nextRefresh) {
                        ...
                        LOG.error("nextRefresh:"
                                + nextRefreshDate
                                + " is in the past: exiting refresh thread. Check"
                    ...
                    if (isUsingTicketCache) {
                        ...
                        while (retry >= 0) {
                            try {
                                LOG.debug("running ticket cache refresh command: "
                                        + cmd + " " + kinitArgs);
                    ...
                    try {
                            ...
                            try {
                                ...
                                if (retry > 0) {
                                    ...
                                    try {
                                        ...
                                        LOG.error(
                                                "Interrupted during login retry after LoginException:",
                                                le);
                                        throw le;
                                    }
                                } else {
                                    LOG.error(
                                            "Could not refresh TGT for principal: "
                                                    + principal + ".", le);
                                }
                            }
                        }
                        ...
                        LOG.error(
                                "Failed to refresh TGT: refresh thread exiting now.",
                                le);
                        break;
                    }
                }
    ...
    private synchronized LoginContext login(final String loginContextName)
        ...
        if (loginContextName == null) {
            throw new LoginException(
                    "loginContext name (JAAS file section header) was null. "
    ...
    private long getRefreshTime(KerberosTicket tgt) {
        ...
        long proposedRefresh = start
                + (long) ((expires - start) * (TICKET_RENEW_WINDOW + (TICKET_RENEW_JITTER * rng
                        .nextDouble())));
    ...
    /**
     * Re-login a principal. This method assumes that {@link #login(String)} has
     * happened already.
     * 
     * @throws javax.security.auth.login.LoginException
     *             on a failure

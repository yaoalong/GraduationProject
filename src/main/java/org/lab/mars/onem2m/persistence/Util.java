...
/**
 * A collection of utility methods for dealing with file name parsing, low level
 * I/O file operations and marshalling/unmarshalling.
 */
public class Util {
	...
	/**
	 * Given two directory files the method returns a well-formed logfile
	 * provider URI. This method is for backward compatibility with the existing
	 * code that only supports logfile persistence and expects these two
	 ...
	 * process of storing a snapshot. Any file that is not a snapshot is also an
	 * invalid snapshot.
	 * 
	 * @param f
	 *            file to verify
	 ...
	 * 
	 * @param f
	 *            output stream to pad
	 * @param currentSize
	 *            application keeps track of the cuurent file size
	 * @param preAllocSize
	...
	/**
	 * Sort the list of files. Recency as determined by the version component of
	 * the file name.
	 *
	 * @param files
	 *            array of files
	 * @param prefix
	 *            files not matching this prefix are assumed to have a version =
	 *            -1)
	 * @param ascending
	 *            true sorted in ascending order, false results in descending
	 *            order

...
public class FileTxnSnapLog {
	...
	public FileTxnSnapLog(File dataDir, File snapDir) throws IOException {
		...
		txnLog = new FileTxnLog( this.dataDir );// 事务log
		snapLog = new FileSnap( this.snapDir );// 快照文件
	}
	...
	public long restore(DataTree dt, Map<Long, Integer> sessions, PlayBackListener listener) throws IOException {
		...
		try {
			while (true) {
				...
				try {
					processTransaction( hdr, dt, sessions, itr.getTxn() );// 处理未处理完成的事务
	...
	public void processTransaction(TxnHeader hdr, DataTree dt, Map<Long, Integer> sessions, Record txn) throws KeeperException.NoNodeException {
		...
		case OpCode.createSession:
			...
			if (LOG.isTraceEnabled()) {
				ZooTrace.logTraceMessage( LOG, ZooTrace.SESSION_TRACE_MASK,
						"playLog --- create session in log: 0x" + Long.toHexString( hdr.getClientId() ) + " with timeout: "
		 ...
		 * there is a chance for later transactions to make into the snapshot.
		 * Then when the snapshot is restored, NONODE/NODEEXISTS errors could
		 * occur. It should be safe to ignore these.
		 */

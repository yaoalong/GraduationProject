 ...
 */
package org.lab.mars.onem2m.persistence;
...
/**
 * This class implements the TxnLog interface. It provides api's to access the
 * txnlogs and add entries to it.
 * <p>
 * The format of a Transactional log is as follows: <blockquote>
 * 
 * <pre>
 * LogFile:
...
public class FileTxnLog implements TxnLog {
	...
	/**
	 * constructor for FileTxnLog. Take the directory where the txnlogs are
	 * stored
	 * 
	 * @param logDir
	 ...
	 * 回滚？ (non-Javadoc)
	 * 
	 * @see org.apache.zookeeper.server.persistence.TxnLog#rollLog()
	 */
	 ...
	 * 关闭所有(non-Javadoc)
	 * 
	 * @see org.apache.zookeeper.server.persistence.TxnLog#close()
	 */
	 ...
	 * @param txn
	 *            the transaction part of the entry returns true iff something
	 *            appended, otw false
	 */
	/*
	 * 添加一个事务到事务日志(non-Javadoc)
	 * 
	 * @see
	 * org.apache.zookeeper.server.persistence.TxnLog#append(org.apache.zookeeper
	 * .txn.TxnHeader, org.apache.jute.Record)
	 */
			...
			if (forceSync) {
				...
				if (syncElapsedMS > fsyncWarningThresholdMS) {
					LOG.warn( "fsync-ing the write ahead log in " + Thread.currentThread().getName() + " took " + syncElapsedMS
							+ "ms which will adversely effect operation latency. " + "See the ZooKeeper troubleshooting guide" );
				}
			}
		}
	...
	/**
	 * a class that keeps track of the position in the input stream. The
	 * position points to offset that has been consumed by the applications. It
	 * can wrap buffered input streams to provide the right offset for the
	 * application.
	 */
	...
	/**
	 * this class implements the txnlog iterator interface which is used for
	...
	public static class FileTxnIterator implements TxnLog.TxnIterator {
		 ...
		 * 
		 * @param logDir
		 *            the transaction database directory
		 * @param zxid
		 ...
		 * 
		 * @return true if there is one and false if there is no new file to be
		 *         read
		 * @throws IOException
		 */
		 ...
		 * 
		 * @param ia
		 *            the inputarchive to be read from
		 * @param is
		 *            the inputstream
		 * @throws IOException
		 */
		protected void inStreamCreated(InputArchive ia, InputStream is) throws IOException {
			...
			if (header.getMagic() != FileTxnLog.TXNLOG_MAGIC) {
				throw new IOException( "Transaction log: " + this.logFile + " has invalid magic number " + header.getMagic() + " != "

...
import org.lab.mars.onem2m.persistence.Util;

/**
 * this class is used to clean up the snapshot and data log dir's. This is
 * usually run as a cronjob on the zookeeper server machine. Invocation of this
 * class will clean up the datalogdir files and snapdir files keeping the last
 * "-n" snapshot files and the corresponding logs.
 */
public class PurgeTxnLog {
    static void printUsage() {
        ...
        System.out
                .println("\tcount -- the number of old snaps/logs you want to keep");
        System.exit(1);
    }
     ...
     * 
     * @param dataDir
     *            the dir that has the logs
     * @param snapDir
     *            the dir that has the snapshots
     * @param num
     *            the number of snapshots to keep
     * @throws IOException
     */
    public static void purge(File dataDir, File snapDir, int num)
            throws IOException {
        if (num < 3) {
        ...
        List<File> files = new ArrayList<File>(Arrays.asList(txnLog
                .getDataDir().listFiles(new MyFileFilter("log."))));
        // add all non-excluded snapshot files to the deletion list
        files.addAll(Arrays.asList(txnLog.getSnapDir().listFiles(
                new MyFileFilter("snapshot."))));
    ...
    /**
     * @param args
     *            PurgeTxnLog dataLogDir dataLogDir -- txn log directory -n num

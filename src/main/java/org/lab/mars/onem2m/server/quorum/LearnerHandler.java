    ...
    public void run() {
        try {
            ...
            if (this.getVersion() < 0x10000) {
                ...
                QuorumPacket newEpochPacket = new QuorumPacket(Leader.LEADERINFO, ZxidUtils.makeZxid(newEpoch, 0), ver);
             ...
             * send a diff to the 
             */ 
    		ReentrantReadWriteLock lock = leader.zk.getZKDatabase().getLogLock();// 主线程还在执行
					...
					if ((maxCommittedLog >= peerLastZxid) && (minCommittedLog <= peerLastZxid)) {
						...
						for (Proposal propose : proposals) {
							...
							if (propose.packet.getZxid() <= peerLastZxid) {
								...
								QuorumPacket qcommit = new QuorumPacket( Leader.COMMIT, propose.packet.getZxid(), null );
								queuePacket( qcommit );
							}
						}
...
//                        }
     ...
     */
    protected void pRequest2Txn(int type,long zxid,M2mRequest nM2mRequest,Record record){

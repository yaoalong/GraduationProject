...
public class LearnerHandler extends Thread {
	...
	public void run() {
		try {
			...
			long lastAcceptedEpoch = ZxidUtils.getEpochFromZxid( qp.getZxid() );// follower的epoch

			long peerLastZxid;
			StateSummary ss = null;
			long zxid = qp.getZxid();
			long newEpoch = leader.getEpochToPropose( this.getSid(), lastAcceptedEpoch );// 大家都在这里进行确认新的newEpoch,获取到了新的newEpoch
			...
			if (this.getVersion() < 0x10000) {
				...
				leader.waitForEpochAck( this.getSid(), ss );// 所有的开始已经确认了，接下来开始进行数据的同步
			}
			peerLastZxid = ss.getLastZxid();

			/* the default to send to the follower */
			int packetToSend = Leader.SNAP;
			long zxidToSend = 0;// 要发送的zxid，用来标记同步的节奏
			...
			try {
				...
				LOG.info( "Synchronizing with Follower sid: " + sid + " maxCommittedLog=0x" + Long.toHexString( maxCommittedLog )
				...
				if (proposals.size() != 0) {
					...
					} else if (peerLastZxid > maxCommittedLog) {// 如果follower比leader的zxid还高的话，那么
						LOG.debug( "Sending TRUNC to follower zxidToSend=0x{} updates=0x{}", Long.toHexString( maxCommittedLog ),
			...
			if (packetToSend == Leader.SNAP) {
				LOG.info( "Sending snapshot last zxid of peer is 0x" + Long.toHexString( peerLastZxid ) + " " + " zxid of leader is 0x"
						+ Long.toHexString( leaderLastZxid ) + "sent zxid of db as 0x" + Long.toHexString( zxidToSend ) );
				// Dump data to peer
	...
	/*
	 * 如果ping失败的话，那么就要去关闭对应的LearnerHandler,以及从leader的learnHandler列表中删除掉
	 */

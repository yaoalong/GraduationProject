    ...
    public void run() {
        ...
        try {
            ...
            while (running) {
                ...
                case LOOKING:
                    LOG.info("LOOKING");
                    System.out.println("进行选举");
                    if (Boolean.getBoolean("readonlymode.enabled")) {
                        LOG.info("Attempting to start ReadOnlyZooKeeperServer");

                        // Create read-only server but don't start it
                        // immediately
                        ...
                        Thread roZkMgr = new Thread() {
                            public void run() {
                                try {
                                    ...
                                    LOG.error(
                                            "FAILED to start ReadOnlyZooKeeperServer",
                                            e);
                                }
                            }
                        };
                        try {
                            ...
                            // If the thread is in the the grace period,
                            // interrupt
                ...
                case FOLLOWING:
                    try {
                        LOG.info("FOLLOWING");
                        System.out.println("进行选举" + "Following");
                        setFollower(makeFollower(logFactory));
                        follower.followLeader();
                    } catch (Exception e) {
                        LOG.warn("Unexpected exception", e);
                    } finally {
                        System.out.println("退出Following");
                        follower.shutdown();
                        setFollower(null);
                        setPeerState(ServerState.LOOKING);
                    }
                ...
                case LEADING:
                    LOG.info("LEADING");
                    System.out.println("进行选举" + "Leading");
                    try {
